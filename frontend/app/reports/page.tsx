...
export default function ReportsPage() {
  ...
  const load = useCallback(async (p: typeof period) => {
    setLoading(true);
    try {
      const res = await cardsAPI.analytics(p);
      setData(res);
    } catch {
      ...
      <div className="page-container" style={{ maxWidth: 1000 }}>
        ...
        <div className="page-header-section" style={{ marginBottom: 'var(--space-6)' }}>
          ...
          <div className="page-header-content">
            ...
            <div className="page-header-text">
              ...
              <p className="page-subtitle">ملخص مالي شامل لجميع بطاقاتك</p>
            </div>
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: 'var(--space-6)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-5)' }}>
            ...
            <h2 style={{ margin: 0, fontSize: '1.05rem' }}>الملخص المالي — {PERIOD_LABELS[period]}</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 'var(--space-4)' }}>
            {[
              ...
              },
              {
                ...
                desc: 'المصروفات ناقص المسترد',
              },
              {
                label: 'إجمالي المدفوع للبنوك',
                value: totals.payments,
                color: 'var(--primary)',
                icon: <CheckCircle size={16} />,
                desc: 'دفعات أُرسلت للبنوك',
              },
              {
              ...
              <div key={item.label} className="card" style={{ padding: 'var(--space-4)', textAlign: 'center', background: 'var(--bg-secondary)' }}>
                ...
                <p style={{ fontSize: '0.73rem', fontWeight: 600, margin: '0 0 2px', color: 'var(--text-primary)' }}>{item.label}</p>
        ...
        {upcoming_payments.length > 0 && (
          <div className="card" style={{ marginBottom: 'var(--space-6)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
              ...
              <div className="card" style={{ padding: 'var(--space-3)', background: 'var(--warning-bg, #fef3c7)', textAlign: 'center' }}>
                  ...
                  <div key={card.card_id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 'var(--space-4)' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
                      ...
                      <div style={{
                        ...
                        background: card.color_hex || 'var(--primary)',
                        flexShrink: 0,
                      }} />
        ...
        {by_card.length > 0 && (
          <div className="card" style={{ marginBottom: 'var(--space-6)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-4)' }}>
              ...
              <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginRight: 'auto', marginLeft: 4 }}>
                اضغط ✏️ لتعديل معدل النقاط لكل بطاقة
              </span>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
              <div className="card" style={{ padding: 'var(--space-4)', background: 'var(--primary-bg)', textAlign: 'center' }}>
                ...
                <p style={{ fontWeight: 700, fontSize: '1.2rem', color: 'var(--primary)', margin: 0 }}>
                  {points_summary.total_earned.toLocaleString('ar')}
                </p>
                <p style={{ fontSize: '0.73rem', color: 'var(--text-secondary)', margin: 0 }}>إجمالي النقاط المكتسبة</p>
              </div>
              <div className="card" style={{ padding: 'var(--space-4)', background: '#fef3c7', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.73rem', color: 'var(--text-secondary)', margin: 0 }}>القيمة التقديرية للنقاط</p>
              </div>
              <div className="card" style={{ padding: 'var(--space-4)', background: 'var(--success-bg)', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.73rem', color: 'var(--text-secondary)', margin: 0 }}>نسبة العائد من الإنفاق</p>
              </div>
            </div>
                ...
                <div key={card.card_id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: 'var(--space-4)' }}>
                    ...
                    {editingCard === card.card_id ? (
                      <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center', flexWrap: 'wrap' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          ...
                          <input
                            type="number" value={editRate} onChange={e => setEditRate(e.target.value)}
                            style={{ width: 70, fontSize: '0.82rem', padding: '3px 6px' }}
                            step="0.1" min="0"
                          />
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                          ...
                          <input
                            type="number" value={editFils} onChange={e => setEditFils(e.target.value)}
                            style={{ width: 70, fontSize: '0.82rem', padding: '3px 6px' }}
                            step="0.1" min="0"
                          />
                        </div>
                        <button onClick={() => savePoints(card.card_id)} disabled={saving} className="btn btn-primary" style={{ padding: '4px 10px', fontSize: '0.78rem' }}>
                          <Check size={13} />
                        </button>
                        <button onClick={() => setEditingCard(null)} className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '0.78rem' }}>
                          <X size={13} />
                        </button>
                      </div>
                    ) : (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 'var(--space-3)', marginTop: 'var(--space-3)' }}>
                    {[
                      ...
                      { label: 'معدل الكسب', value: `${card.points_earn_rate} نقطة/درهم`, color: 'var(--text-primary)' },
                      { label: 'النقاط المكتسبة', value: card.points_earned.toLocaleString('ar'), color: 'var(--primary)' },
                      ...
                      <div key={item.label} style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', padding: 'var(--space-2) var(--space-3)' }}>
                        <p style={{ fontSize: '0.68rem', color: 'var(--text-secondary)', margin: '0 0 2px' }}>{item.label}</p>
                        <p style={{ fontSize: '0.82rem', fontWeight: 700, color: item.color, margin: 0 }}>{item.value}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {by_category.map((cat, idx) => {
                ...
                return (
                  <div key={cat.category} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                    ...
                    <span style={{ minWidth: 120, fontSize: '0.85rem', fontWeight: 500 }}>{cat.category}</span>
                    <MiniBar value={cat.total} max={maxCat} color={color} />
                    <span style={{ minWidth: 90, textAlign: 'end', fontSize: '0.8rem', fontWeight: 600, color }}>
                      {formatAmount(cat.total)}
                    </span>
                    <span style={{ minWidth: 38, textAlign: 'end', fontSize: '0.72rem', color: 'var(--text-secondary)' }}>
                      {pct.toFixed(0)}%
                    </span>
                  </div>
                );
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem', minWidth: 400 }}>
                  <thead>
                    <tr style={{ borderBottom: '2px solid var(--border)' }}>
                      ...
                      <th style={{ padding: 'var(--space-2)', textAlign: 'end', color: 'var(--text-secondary)' }}>الرسم</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Array.from(monthly_trend).reverse().map(row => {
                      ...
                      return (
                        <tr key={row.month} style={{ borderBottom: '1px solid var(--border)' }}>
                          ...
                          <td style={{ padding: 'var(--space-2)', textAlign: 'end', color: 'var(--danger)', fontWeight: 600 }}>
                            {formatAmount(row.purchases)}
                          </td>
                          <td style={{ padding: 'var(--space-2)', textAlign: 'end', color: 'var(--primary)' }}>
                            {row.payments > 0 ? formatAmount(row.payments) : '—'}
                          </td>
                          <td style={{ padding: 'var(--space-2)', textAlign: 'end', color: 'var(--success)' }}>
                          ...
                          <td style={{ padding: 'var(--space-2)', textAlign: 'end' }}>
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, minWidth: 60 }}>
                              <div style={{ flex: 1, height: 4, background: 'var(--bg-secondary)', borderRadius: 99, overflow: 'hidden', width: 50 }}>
        ...
        {/* ── Section 6: Per-card spending summary ── */}
        {by_card.length > 0 && (
          <div className="card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-4)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
              {by_card.map(card => {
                ...
                return (
                  <div key={card.card_id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                    <div style={{ width: 34, height: 22, borderRadius: 3, background: card.color_hex || 'var(--primary)', flexShrink: 0 }} />
