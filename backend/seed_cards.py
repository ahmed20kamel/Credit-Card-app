...
Run with: python manage.py shell < seed_cards.py
Or: python seed_cards.py (after setting up Django)
"""
...
def seed_cards():
    # الحصول على أول مستخدم (أو يمكنك تحديد مستخدم معين)
    ...
    user = users.first()
    print(f'✅ استخدام المستخدم: {user.email}\n')
    print(f'📝 جاري إضافة {len(DUMMY_CARDS)} بطاقة...\n')
    ...
    for i, card_data in enumerate(DUMMY_CARDS, 1):
        try:
            ...
            success_count += 1
            print(f'✅ [{i}/{len(DUMMY_CARDS)}] {card_data["card_name"]} - {card_data["bank_name"]}')
        except Exception as e:
            error_count += 1
            print(f'❌ [{i}/{len(DUMMY_CARDS)}] فشل إضافة {card_data.get("card_name", "Unknown")}: {str(e)}')
